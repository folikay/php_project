...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['comment_submit'])) {
    ...
    // Email details for feedback (optional now)
    $to_feedback = "graceoluwakunmi@gmail.com"; 
    $subject_feedback = "New Comment Feedback";
    $body_feedback = "Comment: $comment";
    $headers_feedback = "From: noreply@osamfem.com"; 
    
    // Try to send email (but don't fail the whole process if it doesn't work)
    $mail_sent = mail($to_feedback, $subject_feedback, $body_feedback, $headers_feedback);
    
    // Always save the comment, regardless of email success
    $comment_entry = date('Y-m-d H:i:s') . " - " . $comment . "\n";
    file_put_contents('comments.txt', $comment_entry, FILE_APPEND | LOCK_EX);
    
    // Store in session for immediate display on this page
    $_SESSION['submitted_comment'] = $comment;
    
    // Set success message
    $comment_success = "Thank you for your feedback! Your comment has been submitted.";
    
    // Optional: Log email failure for debugging (check your server logs)
    if (!$mail_sent) {
        error_log("Email failed to send for comment: $comment");
        // You can still proceed without failing the user
    }
    
    // Redirect to feedback.php after processing (to show on feedback page)
...
<body class="preload">
 ...
 <div class="container">
        <div class="image-section">
            <img src="casestudy4.jpg" alt="Contact Image"> <!-- Replace with your image URL -->
        </div>
        <div class="form-section">
            <h2>Contact Us</h2>
            <?php if (isset($contact_success)) echo "<p class='success'>$contact_success</p>"; ?>
            <?php if (isset($contact_error)) echo "<p class='error'>$contact_error</p>"; ?>
    ...
    <?php if (isset($comment_success)) echo "<p class='success'>$comment_success</p>"; ?>
    <?php if (isset($comment_error)) echo "<p class='error'>$comment_error</p>"; ?>
    <form method="POST" action="">
    ...
    <!-- Display the submitted comment below the form if it exists in session -->
    <?php if (isset($_SESSION['submitted_comment'])): ?>
        <div class="submitted-comment">
            <h4>Your Submitted Comment:</h4>
            <p><?php echo htmlspecialchars($_SESSION['submitted_comment']); ?></p>
            <a href="feedback.php">Visit our feedback page to see more</a>
        </div>
    <?php endif; ?>
    
    <!-- Optionally, display all comments from the file below -->
    <div class="all-comments">
        ...
        if (file_exists('comments.txt')) {
            ...
            if (!empty($comments)) {
                echo '<ul>';
                foreach (array_reverse($comments) as $line) { // Reverse to show newest first
